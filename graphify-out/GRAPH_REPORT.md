...
## Corpus Check
- 170 files · ~190,081 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3092 nodes · 7306 edges · 145 communities (136 shown, 9 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 487 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `efdc4bfb`
...
Cohesion: 0.08
Nodes (67): Dyad, Mcq, BaseModel, Validation for ``frameworks.definition_json`` (PRD §3 and §5). Every…, A 2D canvas on which the respondent places named chips., A multiple-choice question., Every signifier with its kind, in the order the respondent meets them., Reject unknown keys so a typo in the Studio surfaces as an error. (+59 more)
...
### Community 22 - "TestClient"
Cohesion: 0.15
Nodes (29): capture(), MonkeyPatch, Session, TestClient, The browser reads stories; a translation is not one., ``anecdotes.text`` is the record and stays exactly as it was told., Translating into English does not make the story an English story., A translated text is never signified by anyone" (constraint 15). (+21 more)

### Community 23 - "organise.py"
Cohesion: 0.15
Nodes (19): _check_narrative(), _check_tabular(), _hinted_column(), _mock_narrative(), _mock_tabular(), _narrative_prompt(), NarrativeOrganisation, _prose_column() (+11 more)

### Community 24 - "imports.py"
Cohesion: 0.10
Nodes (45): AppError, bad_request(), conflict(), not_found(), The error shape from PRD §4. ``{"error": {"code": ..., "message": plain-English…, An error the operator is meant to read and act on., Something outside the app misbehaved — currently only the AI service., upstream() (+37 more)
...
Cohesion: 0.06
Nodes (35): After the delta — the whole-delta check, Completeness pass, Decisions, Delta phase A, Delta phase B, Delta phase C, Delta phase D, Delta phase E (+27 more)
...
Cohesion: 0.09
Nodes (44): capture(), csv_rows(), MonkeyPatch, parametrize, Session, TestClient, The original language is the record (delta §6, constraint 15). Constraint 15…, A respondent scanning for their language looks for their word, not ours. (+36 more)

### Community 38 - "routers/interpretations.py"
Cohesion: 0.13
Nodes (25): for_framework(), InterpretationIn, InterpretationOut, BaseModel, Session, Collective interpretation: what a room concluded, kept as an artefact.…, Store one conclusion exactly as the room gave it. The text goes in unchanged.…, Every conclusion recorded against these framework versions, newest first. Takes… (+17 more)
...
Cohesion: 0.09
Nodes (42): FrameworkDefinition, The whole respondent-facing definition of one framework version., One id namespace across all signifier kinds — significations key on it., How many signifier screens the respondent will see., PRD §1.1: warn past roughly six signifier screens., chunks(), describe_signifiers(), _prompt() (+34 more)

### Community 49 - "test_schema_absence.py"
Cohesion: 0.14
Nodes (19): _columns(), parametrize, Constraint 9 — respondent anonymity is engineered, not promised. These tests…, No IP, user agent, fingerprint, device/session id or email anywhere., No name-family column on a table whose rows are linked to a respondent., Catch identifiers this test did not anticipate, e.g. ``manager_name``.…, Constraint 9: respondent time is hour-rounded, so no exact clock exists.…, A row written through the ORM gets an hour-rounded stamp automatically. (+11 more)
...
Cohesion: 0.11
Nodes (22): CaptureLink, A token-gated capture URL pointing at one exact framework version., One respondent (or validated AI) placement on one signifier. ``value_json``…, A free-text tag the analyst attaches to a story., Signification, Tag, _anecdote(), _framework() (+14 more)
...
Cohesion: 0.11
Nodes (17): How to resume, Narrative Lens — Latest, Next step, Running it yourself, Starting it: the launcher now sets itself up, The app would not start, and it was our fault, The completeness pass, after Phase 9, The landscape looks like a landscape now (+9 more)
...
Cohesion: 0.12
Nodes (19): answer_counts(), display_title(), known_tags(), marks_for(), BaseModel, Session, The story browser's read model (PRD §1.6). The landscape says where stories…, The delta's display rule, in one place (delta §3). The name its teller gave it… (+11 more)
...
### Community 91 - "backend/landscape.py"
Cohesion: 0.15
...
Cohesion: 0.14
Nodes (25): AiError, Exception, An AI call that failed in a way the operator needs told about. Carries the PRD…, OrganiseError, OrganiseResult, Exception, Stage A's whole output, as stored on the job and shown for confirmation., Stage A produced something that does not match the file. (+17 more)
...
Cohesion: 0.24
Nodes (14): ModuleType, _calls(), _last(), The live path to api.anthropic.com — the one Phase 7 switches on. Everything…, Answer each successive request with the next reply in the list., Strict JSON is asked for on every call, not only where it is convenient., PRD §6 Phase 7: the repair path, exercised through Stage A itself., Every request made, across every client. ``_live_text`` builds a fresh… (+6 more)

### Community 100 - "barycentric.py"
Cohesion: 0.40
Nodes (5): _placed(), point_from_value_json(), Triad barycentric maths. A triad answer is a point inside an equilateral…, A stored answer straight to its point in the triangle. Exactly…, The conversion itself, on weights already known to be usable. Kept apart from…
...
Cohesion: 0.10
Nodes (28): display_name(), What to show for a story's language. A code we know gets its English name. A…, A cached read-time translation of one story into one language. Constraint 15 in…, Translation, cached(), _mock_reply(), Any, BaseModel (+20 more)
...
### Community 106 - "well_formed"
Cohesion: 0.17
Nodes (7): CaptureSettings, Every non-signifier string the respondent reads, plus capture toggles., Well-formed BCP-47, and each offered once. Shape only — no registry lookup. An…, What the welcome screen offers: the configured list, or English., Whether a tag is shaped like a language tag at all., well_formed(), field_validator

### Community 107 - "_live_text"
Cohesion: 0.20
Nodes (9): _live_text(), One live call to api.anthropic.com. The only network in the app. Imported…, _Block, Any, parametrize, A reply may carry blocks that are not text; they are not the answer., _Response, test_every_service_failure_becomes_a_sentence() (+1 more)
...
Cohesion: 0.10
Nodes (33): only_pending(), Select, What counts as data, in one place (constraint 1). An anecdote exists in three…, Narrow a query to the stories still waiting on a person., Anecdote, One story, bound to the exact framework version it was told against.…, counts(), decide() (+25 more)
...
### Community 118 - "routers/landscape.py"
Cohesion: 0.11
Nodes (28): only_validated(), Narrow a query to the stories a person has actually approved. Every read that…, get_clusters(), get_explorer(), get_landscape(), LandscapeSet, BaseModel, Depends (+20 more)
...
### Community 122 - "languages.py"
Cohesion: 0.17
Nodes (15): Language, offered(), BaseModel, The language a story was told in (delta §3, constraint 15). Constraint 15 says…, The languages a framework offers, in the order it lists them. An unknown but…, One language a framework may offer, named twice., LintFinding, One thing worth a second look, and what to try instead. (+7 more)

### Community 123 - "propose.py"
Cohesion: 0.11
Nodes (28): BaseModel, One placement as it arrives from the wizard or paper batch entry., SubmittedSignification, _check_batch(), _mock_batch(), _mock_confidence(), _mock_value(), Placement (+20 more)
...
Cohesion: 0.05
Nodes (83): distance_from_centre(), How far a placement sits from the middle of the triangle. Plain Euclidean…, get_session(), Session, Database engine and session plumbing (constraint 4: SQLite + local files)., FastAPI dependency yielding a session that always closes., mount_frontend(), FastAPI application. Endpoints arrive with the phase that needs them, per PRD… (+75 more)

### Community 127 - "xlsx_bytes"
Cohesion: 0.18
Nodes (12): A two-sheet workbook: one of responses, one lookup table to ignore. The…, xlsx_bytes(), PRD §4: AI endpoints are exempt; reading a job's status is not., test_job_status_is_inside_the_200ms_budget(), Session, TestClient, One run through the whole app, in the order an operator would use it. Every…, The guardrail, followed all the way to the landscape and the exports. (+4 more)
...
### Community 129 - "capture_links.py"
Cohesion: 0.15
...
### Community 132 - "test_translation_readtime.py"
Cohesion: 0.11
Nodes (21): Read-time translation, display-only (delta §6, constraint 15). The second half…, Delta §6: "assert the label is in the same component and not conditionally…, A label with its own state could be turned off; this one has none., The story's own text is rendered before the toggle, always., Nothing to carry across, so nothing to offer., No title, no placements, no group, no provenance. A translator needs words., Translate, not summarise. The register is part of what was said., Shipped with the app, like the linter's — constraint 6 is an app mode. (+13 more)

### Community 133 - "get_translation"
Cohesion: 0.13
Nodes (16): get_translation(), MarksIn, BaseModel, Depends, get, put, Query, Session (+8 more)

### Community 134 - "MonkeyPatch"
Cohesion: 0.24
...
### Community 140 - "ingest_fixtures.py"
Cohesion: 0.25
Nodes (6): pdf_bytes(), _pdf_escape(), pptx_bytes(), Real files, in memory, one per format Narrative Lens claims to read. Acceptance…, A one-page PDF with each paragraph as its own text run. Written by hand:…, vtt_bytes()
...
## Knowledge Gaps
- **212 isolated node(s):** `name`, `version`, `description`, `dev`, `build` (+207 more)
...
- **Why does `Anecdote` connect `Anecdote` to `capture_links.py`, `TestClient`, `TestClient`, `test_translation_readtime.py`, `get_translation`, `test_import_pipeline.py`, `TestClient`, `TestClient`, `test_public_identifier_absence.py`, `validate_definition`, `test_landscape.py`, `imports.py`, `test_language_capture.py`, `test_queue.py`, `test_schema_absence.py`, `Signification`, `models.py`, `backend/stories.py`, `proposed_import`, `Framework`, `AiError`, `test_quality_signals.py`, `translate.py`, `languages.py`, `CaptureError`, `get_session`, `xlsx_bytes`?**
  _High betweenness centrality (0.131) - this node is a cross-community bridge._
- **Why does `FrameworkDefinition` connect `FrameworkDefinition` to `test_edit_log_wording.py`, `validate_definition`, `.estimated_minutes`, `backend/patterns.py`, `imports.py`, `routers/interpretations.py`, `test_design_linter.py`, `backend/exports.py`, `test_placement_shape_parity.py`, `Framework`, `AiError`, `Anecdote`, `public.py`, `clusters.py`, `edit_semantics.py`, `lint.py`, `routers/landscape.py`, `languages.py`, `propose.py`, `get_paper_pack`, `CaptureError`, `get_session`?**
  _High betweenness centrality (0.095) - this node is a cross-community bridge._
- **Why does `make_framework()` connect `make_framework` to `test_quality_signals.py`, `test_live_ai.py`, `test_patterns.py`, `test_language_capture.py`, `MonkeyPatch`, `test_exports.py`, `test_translation_readtime.py`, `test_design_linter.py`, `test_queue.py`, `test_landscape.py`, `test_story_browser.py`, `test_signification_provenance.py`, `proposed_import`, `TestClient`, `test_explorer_clusters.py`?**
